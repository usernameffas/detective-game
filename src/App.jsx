...
const callClaude = async (system, messages, maxTokens = 2000) => {
  const res = await fetch("http://localhost:8787/api/llm", {
    method: "POST",
...
const DUMMY = {
  ...
  truth: "박교수가 오후 10시에 서재 와인에 독을 탔다. 응접실에 있었다는 알리바이는 거짓이다.",
  suspects: {
    "박교수": {
  ...
  clues_public: [
    { id: "c1", icon: "🍷", title: "독이 든 와인잔", content: "피해자의 와인잔에서 독성 물질 검출. 저녁 식사 후 각자 방으로 가져간 것으로, 누군가 오후 10시 이후 서재에 들어와 독을 탄 것으로 추정." },
    { id: "c2", icon: "📄", title: "반쯤 탄 편지", content: "서재 벽난로에서 발견. '...당신의 죄를 세상에 알리겠소. 내일까지 자백하지 않으면...' 이라는 문구만 읽힘." },
  ],
  clues_hidden: [
    {
      id: "h1", icon: "👞", title: "정원 진흙 발자국",
      content: "서재 입구 카펫에서 진흙 묻은 발자국 발견. 오후 8시 이후 비가 내렸고, 정원을 지나야만 진흙이 묻을 수 있음.",
      unlock_suspect: "박교수",
      unlock_hint: "그날 밤 동선",
    ...
    {
      id: "h2", icon: "📓", title: "피해자의 비밀 일기",
      content: "피해자 서랍에서 발견. '그가 내 원고 전체를 훔쳤다. 오늘 밤 담판을 짓겠다.' 날짜는 사망 당일.",
      unlock_suspect: "최부인",
    ...
    {
      id: "h3", icon: "📚", title: "응접실 먼지 쌓인 책",
      content: "박교수가 읽었다는 응접실 소파 옆 책에 두꺼운 먼지가 쌓여있음. 최근 누가 읽은 흔적이 없음.",
      unlock_suspect: "김청년",
...
export default function App() {
  ...
  const startGame = async () => {
    setError(null); setDebugLog([]); setPhase(PHASE.LOADING);
    log("API 호출 시작...");
    try {
...
{"case_title":"제목","case_description":"짧은설명","location":"장소","time_of_death":"오후 10시~11시","culprit":"용의자C이름","motive":"동기","truth":"범인이 어떻게 범행했는지 한 줄 진실","suspects":{"용의자A":{"personality":"성격","alibi":"사망시간대 알리바이(목격자있음)","secret":"비밀","is_culprit":false},"용의자B":{"personality":"성격","alibi":"사망시간대 알리바이(목격자있음)","secret":"비밀","is_culprit":false},"용의자C":{"personality":"성격","alibi":"사망시간대 알리바이(목격자없음,허점있음)","secret":"범행비밀","is_culprit":true}},"clues_public":[{"id":"c1","icon":"🔍","title":"단서제목","content":"공개단서1"},{"id":"c2","icon":"📄","title":"단서제목","content":"공개단서2"}],"clues_hidden":[{"id":"h1","icon":"👞","title":"단서제목","content":"숨겨진단서1","unlock_suspect":"용의자A","unlock_hint":"어떤질문에서나오는지","contradiction":"범인알리바이와모순","target_suspect":"용의자C"},{"id":"h2","icon":"📓","title":"단서제목","content":"숨겨진단서2","unlock_suspect":"용의자B","unlock_hint":"어떤질문에서나오는지","contradiction":null,"target_suspect":"용의자C"},{"id":"h3","icon":"📚","title":"단서제목","content":"숨겨진단서3","unlock_suspect":"용의자C","unlock_hint":"어떤질문에서나오는지","contradiction":"범인알리바이와모순","target_suspect":"용의자C"}],"winning_clues":["h1","h3"]}
...
- contradiction이 있는 단서는 반드시 범인 알리바이를 논리적으로 반박해야 함`
  ...
  const checkHiddenClues = (suspectName, message) => {
    ...
    scenario.clues_hidden.forEach(clue => {
      if (clue.unlock_suspect === suspectName && !foundClues.includes(clue.id)) {
...
${unlockableClue ? `질문이 "${unlockableClue.unlock_hint}"와 관련되면 자연스럽게 "${unlockableClue.content}"에 대한 정보를 흘리세요.` : ""}
등장인물 외 인물 언급 금지. 2~3문장 한국어로 답하세요.`,
  ...
  const presentClue = async (clueId) => {
    ...
    const allClues = [...(scenario.clues_public || []), ...(scenario.clues_hidden || [])];
    const clue = allClues.find(c => c.id === clueId);
    if (!clue) return;
    setPresentingClue(clueId);
    setIsThinking(true);
    const suspect = scenario.suspects[selectedSuspect];
    const isContradiction = clue.target_suspect === selectedSuspect && clue.contradiction;
    try {
      const reply = await callClaude(
        `당신은 추리 게임 용의자 ${selectedSuspect}입니다.
성격: ${suspect.personality}
...
${isContradiction
  ...
  : `이 물증은 당신과 직접 관련 없습니다. 자연스럽게 반응하세요.`}`,
        [...histories[selectedSuspect], { role: "user", content: `[물증 제시: ${clue.title}] "${clue.content}"` }]
      );
      setHistories(prev => ({
        ...prev,
        [selectedSuspect]: [
          ...
          { role: "assistant", content: reply + (isContradiction ? " ⚠️" : "") }
        ]
      }));
    } catch (e) { setError(e.message); }
    setIsThinking(false);
  ...
  const reset = () => {
    setScenario(null); setResult(null); setHistories({});
    setSelectedSuspect(null); setError(null); setDebugLog([]);
    setFoundClues([]); setNewClueAlert(null); setShowClues(false);
    setPhase(PHASE.TITLE);
  };

  const suspects = scenario ? Object.keys(scenario.suspects) : [];
  const allClues = scenario ? [...scenario.clues_public, ...scenario.clues_hidden.filter(c => foundClues.includes(c.id))] : [];
  ...
  const S = {
    ...
    btnRed: (disabled) => ({ background: disabled ? "rgba(80,80,80,0.2)" : "rgba(160,60,60,0.2)", border: `1px solid ${disabled ? "#555" : "#a03c3c"}`, color: disabled ? "#666" : "#e08080", padding: "10px 22px", cursor: disabled ? "not-allowed" : "pointer", fontFamily: "Georgia, serif", fontSize: 14, borderRadius: 3 }),
    ...
    clueCard: (presenting) => ({ background: presenting ? "rgba(139,90,43,0.2)" : "rgba(19,14,9,0.8)", border: `1px solid ${presenting ? "#8b6a2f" : "#3a2a15"}`, borderRadius: 4, padding: "8px 12px", marginBottom: 8, cursor: "pointer", transition: "all 0.2s" }),
  };
  ...
  return (
    <div style={S.root}>
      <div style={S.wrap}>
        ...
        <p style={{ textAlign: "center", color: "#8b7355", fontSize: 13, marginBottom: 24, fontStyle: "italic" }}>단서를 모아 범인을 밝혀내세요</p>

        {/* 새 단서 알림 */}
        {newClueAlert && (
          <div style={{ background: "#1a2a1a", border: "1px solid #4a8a4a", color: "#8aca8a", padding: "12px 16px", borderRadius: 4, marginBottom: 14, fontSize: 13, textAlign: "center" }}>
            🔍 새 단서 발견: <b>{newClueAlert.title}</b>
          </div>
        )}

        {error && (
          <div style={{ background: "#2a0a0a", border: "1px solid #a00", color: "#f99", padding: 12, borderRadius: 4, marginBottom: 14, fontSize: 12, fontFamily: "monospace" }}>
            <b>오류:</b> {error}
            <button onClick={() => setError(null)} style={{ marginLeft: 10, background: "none", border: "none", color: "#f99", cursor: "pointer" }}>✕</button>
          </div>
        )}
        {debugLog.length > 0 && (
          <div style={{ background: "#0a0f0a", border: "1px solid #1a3a1a", color: "#6a9a6a", padding: 10, borderRadius: 4, marginBottom: 14, fontSize: 11, fontFamily: "monospace" }}>
        ...
        {phase === PHASE.TITLE && (
          <div style={{ ...S.card, textAlign: "center", padding: 40 }}>
            ...
            <div style={{ color: "#8b7355", fontSize: 13, marginBottom: 28, lineHeight: 1.7 }}>
              용의자를 심문하고 단서를 수집하세요<br />충분한 증거가 모이면 범인을 지목할 수 있습니다
            </div>
            <button style={S.btnBig} onClick={startGame}>🎲 AI 시나리오로 시작</button>
            <div style={{ marginTop: 14 }}>
              <button style={S.btnGray} onClick={() => loadScenario(DUMMY)}>🗒️ 샘플 시나리오로 시작</button>
            </div>
          </div>
        )}
          ...
          <>
            ...
            <div style={S.card}>
              ...
              {scenario.clues_public.map(c => (
                <div key={c.id} style={{ marginBottom: 12, paddingLeft: 10, borderLeft: "2px solid #3a2a15" }}>
                  <div style={{ color: "#8b6a2f", fontSize: 12, marginBottom: 3 }}>{c.icon} {c.title}</div>
                  <div style={{ color: "#c4a96e", fontSize: 13, lineHeight: 1.7 }}>{c.content}</div>
                </div>
              ))}
              <div style={{ color: "#5a4a35", fontSize: 11, fontStyle: "italic", marginTop: 8 }}>
        ...
        {phase === PHASE.INTERROGATE && scenario && (
          <>
            ...
            <div style={{ background: "#130e09", border: "1px solid #3a2a15", borderRadius: 4, marginBottom: 14, overflow: "hidden" }}>
              <button onClick={() => setShowClues(o => !o)} style={{ width: "100%", textAlign: "left", background: "none", border: "none", color: "#8b6a2f", padding: "12px 20px", cursor: "pointer", fontFamily: "monospace", fontSize: 10, letterSpacing: 3, textTransform: "uppercase", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span>📋 수집한 단서 ({foundClues.length}개)</span>
                <span>{showClues ? "▲" : "▼"}</span>
              </button>
              {showClues && (
                <div style={{ padding: "0 16px 16px", borderTop: "1px solid #3a2a15" }}>
                  <div style={{ color: "#5a4a35", fontSize: 11, fontStyle: "italic", margin: "10px 0 12px" }}>
                    단서를 클릭하면 현재 용의자에게 제시합니다
                  </div>
                  {allClues.map(c => (
                    <div key={c.id} style={S.clueCard(presentingClue === c.id)} onClick={() => presentClue(c.id)}>
                      <div style={{ color: "#8b6a2f", fontSize: 11, marginBottom: 3 }}>{c.icon} {c.title}</div>
                      <div style={{ color: "#b09870", fontSize: 12, lineHeight: 1.6 }}>{c.content}</div>
                    </div>
                  ))}
                  {!canAccuse() && (
                    <div style={{ color: "#5a4a35", fontSize: 11, fontStyle: "italic", marginTop: 8 }}>
                  ...
                  {canAccuse() && (
                    <div style={{ color: "#7ab88a", fontSize: 12, marginTop: 8 }}>
              ...
              <div style={S.card}>
                ...
                <div style={{ minHeight: 180, maxHeight: 320, overflowY: "auto", marginBottom: 12 }}>
                  {histories[selectedSuspect].length === 0 && (
                    <div style={{ color: "#5a4a35", fontStyle: "italic", fontSize: 13 }}>질문을 입력하거나 단서를 제시하세요...</div>
                  )}
                  {histories[selectedSuspect].map((m, i) => (
                    <div key={i} style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 10, color: m.role === "user" ? "#5a8a6a" : "#8b6a2f", marginBottom: 3, letterSpacing: 1 }}>
                        {m.role === "user" ? "탐정" : selectedSuspect}
                      </div>
                      <div style={{ fontSize: 14, lineHeight: 1.7, padding: "8px 12px", borderLeft: `2px solid ${m.role === "user" ? "#5a8a6a" : "#8b6a2f"}`, color: m.role === "user" ? "#a8c8b0" : "#c4a96e" }}>
                        {m.content}
                      </div>
                    </div>
                  ))}
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <input style={S.input} placeholder="질문을 입력하세요 (Enter)" value={input}
            ...
            <div style={{ textAlign: "center" }}>
              <button style={S.btnRed(!canAccuse())} onClick={() => canAccuse() && setPhase(PHASE.ACCUSE)}>
        ...
        {phase === PHASE.ACCUSE && scenario && (
          <div style={S.card}>
            ...
            <div style={{ color: "#8b7355", fontSize: 13, marginBottom: 8, fontStyle: "italic" }}>범인이라고 생각하는 사람을 선택하세요</div>
            <div style={{ color: "#5a4a35", fontSize: 11, marginBottom: 16 }}>수집한 단서: {allClues.map(c => c.title).join(", ")}</div>
            {suspects.map(n => (
              <button key={n} onClick={() => accuse(n)} style={{ display: "block", width: "100%", textAlign: "left", background: "rgba(160,60,60,0.1)", border: "1px solid rgba(160,60,60,0.3)", color: "#e8d5b0", padding: "12px 16px", cursor: "pointer", fontFamily: "Georgia, serif", fontSize: 15, borderRadius: 3, marginBottom: 8 }}>
                {n}
              </button>
            ))}
        ...
        {phase === PHASE.RESULT && scenario && result && (
          <div style={{ ...S.card, textAlign: "center", padding: 36 }}>
            ...
            <div style={{ fontSize: 22, color: result.correct ? "#7ab88a" : "#c07070", marginBottom: 10 }}>
              {result.correct ? "사건 해결!" : "수사 실패"}
            </div>
            <div style={{ color: "#b09870", fontSize: 14, marginBottom: 6 }}>
              {result.correct ? `정확합니다! 범인은 ${scenario.culprit}이었습니다.` : `틀렸습니다. 범인은 ${scenario.culprit}이었습니다.`}
            </div>
            <div style={{ color: "#8b7355", fontStyle: "italic", fontSize: 13, margin: "14px 0 24px", lineHeight: 1.7 }}>
              🔓 {scenario.motive}
            </div>
